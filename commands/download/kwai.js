...
export const description = "Baixa video do Kwai";
export const uso = "/kwai https://kwai.com/@user/video/ID";
export const cooldown = 5;
export default async function cmdkwai(ctx) {
  const url = extrairUrl(ctx.q) || ctx.q.trim();
  if (!url) {
    await ctx.uso(
      "/kwai https://kwai.com/@user/video/ID",
      "Manda o link junto com o comando.",
    );
    return;
  }
  if (!links.kwai(url)) {
    await ctx.erro("Esse link nao e do Kwai.");
    return;
  }
  const carregando = await ctx.carregando(" Baixando...");
  try {
    const bruto = await download.kwai(url);
