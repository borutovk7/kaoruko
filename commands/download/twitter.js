...
export const description = "Baixa video do X (Twitter)";
export const aliases = ["x", "xdl"];
export const uso = "/twitter https://x.com/user/status/ID";
export const cooldown = 5;
export default async function cmdtwitter(ctx) {
  ...
  if (!links.twitter(url)) {
    await ctx.erro("Esse link nao e do X/Twitter.");
    return;
  }
  const carregando = await ctx.carregando(" Baixando...");
  try {
    const bruto = await download.twitter(url);
