...
export const description = "Baixa video do CapCut";
export const uso = "/capcut https://www.capcut.com/t/ID";
export const cooldown = 5;
export default async function cmdcapcut(ctx) {
  const url = extrairUrl(ctx.q) || ctx.q.trim();
  if (!url) {
    await ctx.uso(
      "/capcut https://www.capcut.com/t/ID",
  ...
  try {
    const bruto = await download.capcut(url);
