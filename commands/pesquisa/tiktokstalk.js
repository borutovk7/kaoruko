...
export const description = 'Informacoes de um perfil do TikTok';
export const aliases = ['ttstalk'];
export const uso = '/tiktokstalk tiktok';
export const cooldown = 5;
export default async function cmdtiktokstalk(ctx) {
  if (!ctx.q) {
    await ctx.uso('/tiktokstalk tiktok', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.tiktokStalk(ctx.q);
    ...
    const linhas = ['<b>INFORMACOES DE UM PERFIL DO TIKTOK</b>', ''];
