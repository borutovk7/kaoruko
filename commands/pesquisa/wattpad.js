...
export const description = 'Pesquisa historias no Wattpad';
export const uso = '/wattpad romance';
export const cooldown = 5;
export default async function cmdwattpad(ctx) {
  if (!ctx.q) {
    await ctx.uso('/wattpad romance', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.wattpad(ctx.q);
    ...
    const linhas = ['<b>PESQUISA HISTORIAS NO WATTPAD</b>', ''];
