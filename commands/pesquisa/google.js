...
export const description = 'Pesquisa no Google';
export const uso = '/google receita de bolo';
export const cooldown = 5;
export default async function cmdgoogle(ctx) {
  if (!ctx.q) {
    await ctx.uso('/google receita de bolo', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.google(ctx.q);
    ...
    const linhas = ['<b>PESQUISA NO GOOGLE</b>', ''];
