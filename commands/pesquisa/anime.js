...
export const description = 'Pesquisa animes';
export const aliases = ['sanime'];
export const uso = '/anime naruto';
export const cooldown = 5;
export default async function cmdanime(ctx) {
  if (!ctx.q) {
    await ctx.uso('/anime naruto', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.anime(ctx.q);
    ...
    const linhas = ['<b>PESQUISA ANIMES</b>', ''];
