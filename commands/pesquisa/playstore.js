...
export const description = 'Pesquisa apps na Play Store';
export const aliases = ['ps'];
export const uso = '/playstore whatsapp';
export const cooldown = 5;
export default async function cmdplaystore(ctx) {
  if (!ctx.q) {
    await ctx.uso('/playstore whatsapp', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.playstore(ctx.q);
    ...
    const linhas = ['<b>PESQUISA APPS NA PLAY STORE</b>', ''];
