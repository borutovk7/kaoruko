...
export const description = 'Pesquisa no Mercado Livre';
export const aliases = ['ml'];
export const uso = '/mercadolivre teclado';
export const cooldown = 5;
export default async function cmdmercadolivre(ctx) {
  if (!ctx.q) {
    await ctx.uso('/mercadolivre teclado', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.mercadolivre(ctx.q);
    ...
    const linhas = ['<b>PESQUISA NO MERCADO LIVRE</b>', ''];
