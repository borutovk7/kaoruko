...
export const description = 'Pesquisa apps modificados';
export const uso = '/happymod minecraft';
export const cooldown = 5;
export default async function cmdhappymod(ctx) {
  if (!ctx.q) {
    await ctx.uso('/happymod minecraft', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.happymod(ctx.q);
    ...
    const linhas = ['<b>PESQUISA APPS MODIFICADOS</b>', ''];
