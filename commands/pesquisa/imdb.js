...
export const description = 'Pesquisa filmes e series no IMDb';
export const aliases = ['filme', 'movie'];
export const uso = '/imdb interstellar';
export const cooldown = 5;
export default async function cmdimdb(ctx) {
  if (!ctx.q) {
    await ctx.uso('/imdb interstellar', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.imdb(ctx.q);
    ...
    const linhas = ['<b>PESQUISA FILMES E SERIES NO IMDB</b>', ''];
