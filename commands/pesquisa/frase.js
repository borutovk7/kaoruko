...
export const description = 'Frases do Pensador';
export const aliases = ['pensador'];
export const uso = '/frase amor';
export const cooldown = 5;
export default async function cmdfrase(ctx) {
  if (!ctx.q) {
    await ctx.uso('/frase amor', 'Digite o que voce quer buscar.');
    return;
  }
  const carregando = await ctx.carregando(' Buscando...');
  try {
    const bruto = await pesquisa.pensador(ctx.q);
    ...
    const linhas = ['<b>FRASES DO PENSADOR</b>', ''];
